...
export interface ApkFilesResource {
  path: string, // 파일명을 제외한 로컬 저장소 폴더 경로
  url: string, // 서버에 있는 리소스 url  ex) 'http://i.torder.com/audio/greeting_구로통닭.mp3'
  fileName: string, // 파일명 'greeting_구로통닭.mp3'
  fileSize: number, // byte
...
export interface ApkUpdateOptions {
  remove: number,
  updateTime: number,
  version: string,
}
...
export interface NativeBridgeInterface {
   ...
   * | v1.8.00 | 최초 생성 |
   *
   * 테블릿의 티오더 앱 apk 버전을 업데이트 합니다.
   *
   * * @param {object} updateFile 다운로드 받을 apk file 명세 객체
   * * @param {object} option 앱 다운로드 option 설정
   */
  selfUpdate: (updateFile: ApkFilesResource, option: ApkUpdateOptions) => void,
