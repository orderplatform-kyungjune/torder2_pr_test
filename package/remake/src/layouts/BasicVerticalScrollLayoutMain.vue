...
const themeType = computed(() => themeStore.theme?.type);
...
const themeColor = computed(() => (themeStore.theme?.color));
...
watch(() => init.value.storeInfo, () => {
  // 스크롤시 장바구니 닫히는 기능 장바구니 덮기 사용시에만 동작
  if (init.value.storeInfo.cartPageOverride === 1) {
