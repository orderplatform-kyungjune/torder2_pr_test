...
import { productOptionItemType } from '@torder/common/interface/product';
import {
  cartProductType,
...
const isGroupSelectedItem = (groups: productOptionItemType[]) =>
  groups.some((group) =>
...
const setActiveOptionIndex = (index: number) => {
  document
    .querySelector('.image-option-detail-item-container')
    ?.scrollTo({ top: 0 });
...
const onConfirmSheet = () => {
  ...
  if (result) {
    const getGroupType = targetProductInfo.value
      .options as unknown as CartProductOptionGroupType[];
    const getItemType = getGroupType?.[activeOptionGroupIndex.value]
...
const getDividerCount = (items: cartProductType[], index: number) => {
...
const getGroupItemRange = computed(() => {
  const targetOptionGroup = targetProductInfo.value?.options?.[
    activeOptionGroupIndex.value
  ] as unknown as CartProductOptionGroupType;
...
const checkOptionValidation = () => {
  ...
  const optionData =
    optionCartItems.value as unknown as CartProductOptionGroupType[];
...
const setIntersectionObserver = () => {
  ...
  const options = {
    root: document.querySelector('.image-option-group-list'),
    rootMargin: '0px',
    threshold: 0.8,
  };
...
const isDepthOption = (optionItem: cartProductType) =>
  optionItem.items && optionItem.items.length > 0;
...
<template>
  <div class="image-option-container">
    <div class="image-option-body">
      <div
        class="image-option-list"
        :style="{ 'background-image': `url(${categoryBackgroundImage})` }"
      >
        <div class="image-option-title">
          {{ t('옵션 선택') }}
        </div>
        <div
          v-observer
          class="image-option-group-list"
        >
          <div
          ...
          >
            <div
              ...
              class="image-option-group-active"
            >
              <div class="image-option-group-active-radius-box">
                <div
                  class="image-option-group-active-radius-top"
                  :class="getDarkModeClass()"
                ></div>
              </div>
              <div
                class="image-option-group-active-container"
                :class="getDarkModeClass()"
              >
                <div
                  v-if="optionGroup.require === 1"
                  class="image-option-group-active-essential"
                  :class="getDarkModeClass()"
                >
                  {{ t('필수') }}
                </div>
                <div class="image-option-group-active-name ellipsis-text-3">
                  {{
                    removeNewLine(
                      translateLanguage(
              ...
              <div class="image-option-group-active-radius-box">
                <div
                  class="image-option-group-active-radius-bottom"
                  :class="getDarkModeClass()"
                ></div>
              </div>
            </div>
            <div
              v-else
              class="image-option-group-box"
              @click="setActiveOptionIndex(groupIndex)"
            >
              <div
                v-if="optionGroup.require === 1"
                class="image-option-group-box-essential"
              >
                {{ t('필수') }}
              </div>
              <p class="image-option-group-name ellipsis-text-2">
                {{
                  removeNewLine(
      ...
      <div
        class="image-option-detail"
        :class="getDarkModeClass()"
      >
        <div class="image-option-detail-title">
          <div
            class="image-option-detail-name"
            :class="getDarkModeClass()"
          >
            {{
              removeNewLine(
          ...
          <div class="image-option-detail-limit">
            {{ getGroupItemRange }}
          </div>
        </div>
        <div class="image-option-detail-item-container">
          <div
            v-for="(optionItem, itemIndex) in getTargetItemsInfo"
            :key="getOptionItemKey(optionItem, itemIndex)"
            class="image-option-detail-item-wrap"
            :class="{ 'display-none': optionItem.isPreSet }"
            @click="
              clickOptionItem(getActiveOptionItemList, optionItem, itemIndex)
            "
          >
            <div v-if="!optionItem.parentName && !optionItem.isPreSet">
              <div class="image-option-detail-item-image-wrap">
                <div
                  v-if="isDepthOption(optionItem)"
                  class="image-option-detail-item-image-label"
                >
                  {{ t('옵션') }}
                </div>
                <img
                  v-if="optionItem.image"
                  class="image-option-detail-item-image"
                  :class="getDarkModeClass()"
                  :src="optionItem.image"
                />
                <div
                  v-else
                  class="image-option-detail-item-image-empty"
                  :class="getDarkModeClass()"
                >
                  <img :src="getCustomThemeProductLogoImage()" />
                </div>
                <div
                  v-if="optionItem.isSale === 0"
                  class="image-option-detail-item-sold-out"
                >
                  {{ t('품절') }}
                </div>
              </div>
              <div
                class="image-option-detail-item-title ellipsis-text-2"
                :class="getDarkModeClass()"
              >
              ...
              <div
                ...
                class="image-option-detail-item-price"
                :class="getDarkModeClass()"
              >
              ...
              <div
                v-if="optionItem.code === 'not_select'"
                class="image-option-detail-item-not-select"
                :class="[{ select: optionItem.checked }, getDarkModeClass()]"
              >
                <CheckBox
                ...
                />
                <span class="image-option-detail-item-not-select-text">
                  {{ t('선택 안 함') }}
                </span>
              </div>
              <div
                v-else-if="optionItem.checked && optionItem.items?.length === 0"
                class="image-option-detail-item-checked"
              >
                <div
                  v-if="
                  ...
                  "
                  class="image-option-detail-item-checked-function"
                >
                  <GrayMinus
                    class="image-option-detail-item-checked-button"
                    @click.stop="
                      () =>
                    ...
                    "
                  />
                  <span class="image-option-detail-item-checked-number">
                ...
                <div
                  ...
                  class="image-option-detail-item-not-select"
                  :class="[
                    { select: optionItem.code !== 'not_select' },
                    getDarkModeClass(),
                  ]"
                >
                  <CheckBox
      ...
      >
        <div
          v-if="
            optionCartItems &&
            isGroupSelectedItem(
              optionCartItems as unknown as productOptionItemType[],
            )
          "
        >
          <div
          ...
          >
            <div
              v-if="isOptionSelectedItem(selectGroup)"
              class="image-option-bill-list-group"
              :class="getDarkModeClass()"
            >
              <div
                class="image-option-bill-list-group-name"
                :class="getDarkModeClass()"
              >
                {{
                  removeNewLine(
              ...
              >
                <div
                  ...
                  class="image-option-bill-list-item"
                >
                  <div class="image-option-bill-list-item-name">
                    <div
                      class="image-option-bill-list-item-title ellipsis-text-3"
                      :class="getDarkModeClass()"
                    >
                      {{
                        removeNewLine(
                    ...
                    <div
                      ...
                      class="image-option-bill-list-item-delete"
                      :class="getDarkModeClass()"
                      @click.stop="deleteOptionItem(selectItem, selectGroup)"
                    >
                      {{ t('삭제') }}
                    </div>
                  </div>
                  ...
                  >
                    ...
                    >
                      {{
                        checkStandardPriceUnit(
                          getPriceTotalItem(selectItem).toLocaleString(),
                        )
                      }}
                    </p>
                    <button
                ...
                <div
                  v-if="
                    selectItem.checked &&
                    getDividerCount(selectGroup?.items, itemIndex)
                  "
    ...
    <div
      class="image-option-footer"
      :class="getDarkModeClass()"
    >
      <div
        class="image-option-footer-title"
        :class="getDarkModeClass()"
      >
        {{
          removeNewLine(
      ...
      <div class="image-option-footer-button-wrap">
        <div
          class="image-option-footer-count-close"
          @click="pushPage(home)"
        >
          {{ t('닫기') }}
        </div>
        <div
          class="image-option-footer-confirm-button"
...
.image-option-container {
  ...
  .image-option-body {
    ...
    .image-option-list {
      ...
      .image-option-group-list {
        ...
        .image-option-group-active {
          display: flex;
          flex-direction: column;
          padding: 0.7813vw 0 0.7813vw 1.5625vw;

          .image-option-group-active-radius-box {
            display: flex;
            justify-content: flex-end;
            width: 100%;

            .image-option-group-active-radius-top {
              width: 1.5625vw;
              height: 1.5625vw;
            ...
            .image-option-group-active-radius-bottom {
              width: 1.5625vw;
              height: 1.5625vw;
              background-color: inherit;
          ...
          .image-option-group-active-container {
            display: flex;
            flex-direction: column;
            gap: 0.9375vw;
            padding: 1.5625vw 0.7813vw 1.5625vw 1.25vw;
            background-color: #fff;
            ...
            .image-option-group-active-essential {
              display: flex;
              align-items: center;
            ...
            .image-option-group-active-name {
              width: 14.0625vw;
              font-size: 1.875vw;
              font-weight: 500;
              line-height: 1.3;
        ...
        .image-option-group-box {
          display: flex;
          flex-direction: column;
          gap: 0.9375vw;
          padding: 1.875vw 0 1.875vw 1.5625vw;

          .image-option-group-box-essential {
            display: flex;
            align-items: center;
            justify-content: center;
            width: min-content;
    ...
    .image-option-detail {
      ...
      .image-option-detail-title {
        display: flex;
        align-items: center;
        justify-content: space-between;

        .image-option-detail-name {
          width: 32.5vw;
          overflow: hidden;
          font-size: 2.0313vw;
          font-weight: 700;
      ...
      .image-option-detail-item-container {
        display: grid;
        grid-template-columns: 1fr 1fr;
        gap: 1.875vw 1.4063vw;
        overflow-y: scroll;

        .image-option-detail-item-wrap {
          position: relative;
          display: flex;
          flex-direction: column;
          align-items: center;
          ...
          .image-option-detail-item-image-wrap {
            position: relative;
            width: 22.3438vw;
            height: 16.7188vw;

            .image-option-detail-item-image-label {
              position: absolute;
              top: 0;
              right: 0;
              display: flex;
            ...
            .image-option-detail-item-image {
              width: 100%;
              height: 100%;
              background-color: #818181;
              background-image: url(../../assets/icn_network_error.svg);
              background-repeat: no-repeat;
              background-position: center;
              background-size: 6.3281vw 4.2188vw;
              border: 0.0781vw solid #d5d5d5;
              border-radius: 1.1172vw;
            ...
            .image-option-detail-item-image-empty {
              display: flex;
              align-items: center;
              justify-content: center;
              width: 100%;
            ...
            .image-option-detail-item-sold-out {
              ...
              border-bottom-right-radius: 1.1172vw;
              border-bottom-left-radius: 1.1172vw;
            }
          }

          .image-option-detail-item-title {
            width: 100%;
            margin: 1.1172vw 0 0.6984vw 0;
            font-size: 1.875vw;
            font-weight: 400;
            line-height: normal;
            color: #5b5b5b;
          ...
          .image-option-detail-item-price {
            width: 100%;
            font-size: 1.875vw;
            font-weight: 700;
            line-height: normal;
          ...
          .image-option-detail-item-not-select {
            position: absolute;
            top: 0;
            left: 0;
            display: flex;
            ...
            .image-option-detail-item-not-select-text {
              font-size: 2.1875vw;
              font-weight: 700;
              line-height: normal;
              color: #fff;
          ...
          .image-option-detail-item-checked {
            position: absolute;
            top: 0;
            left: 0;
            display: flex;
            ...
            .image-option-detail-item-checked-function {
              display: flex;
              justify-content: space-between;
              width: 100%;
              padding: 0 1.5625vw;

              .image-option-detail-item-checked-number {
                display: flex;
                align-items: center;
                font-size: 2.5vw;
                font-weight: 700;
              ...
              .image-option-detail-item-checked-button {
                width: 4.6875vw;
                height: 3.9063vw;
                border: 0.0781vw solid #959595;
                border-radius: 0.625vw;
                box-shadow: 0 0.2604vw 0.2604vw 0 rgba(0, 0, 0, 0.25);
              }
            }
          }
        }
    ...
    .image-option-bill {
      ...
      .image-option-bill-list-group {
        ...
        .image-option-bill-list-group-name {
          width: 25.9375vw;
          margin-bottom: 0.78125vw;
          overflow: hidden;
          font-size: 1.5625vw;
        ...
        .image-option-bill-list-item {
          display: flex;
          flex-direction: column;
          gap: 1.25vw;

          .image-option-bill-list-item-name {
            display: flex;
            gap: 1.5625vw;
            align-items: flex-start;
            justify-content: space-between;

            .image-option-bill-list-item-title {
              width: 18.75vw;
              font-size: 1.875vw;
              font-weight: 500;
              line-height: 1.3;
            ...
            .image-option-bill-list-item-delete {
              display: flex;
              align-items: center;
              justify-content: center;
              padding: 0.625vw 1.25vw;
              ...
              background-color: #fff;
              border: 0.1171875vw solid v-bind(buttonBackgroundColor);
  ...
  .image-option-footer {
    ...
    .image-option-footer-button-wrap {
      display: flex;
      gap: 0.9375vw;
      align-items: center;
      justify-content: space-between;

      .image-option-footer-count-close {
        display: flex;
        align-items: center;
        justify-content: center;
      ...
      .image-option-footer-confirm-button {
        display: flex;
        align-items: center;
        justify-content: center;
        width: 27.5vw;
